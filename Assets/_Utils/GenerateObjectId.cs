...
public static class GenerateObjectId
{
    ...
    public static string GenerateItemId(Item item)
    {
        return $"{item.itemIndex}_" +
                $"{item.transform.position.x.ToString("F2")}_" +
                $"{item.transform.position.y.ToString("F2")}_" +
                $"{item.transform.position.z.ToString("F2")}_" +
                $"{item.transform.rotation.eulerAngles.y.ToString("F2")}_";
    }
}

...
public class PerfectLookAt:MonoBehaviour
{
	...
	private void Update()
	{
		if (m_LookAtBones.Length > 0) {
			if (m_DrawDebugLookAtLines) {
				Vector3 destination = m_LookatTarget.transform.position - m_LookAtBones[0].m_Bone.position;
	...
	private void LateUpdate()
	{
		...
		if (m_LookAtBones.Length > 0) {
			...
			Vector3 targetVector = m_LookatTarget.transform.position - m_LookAtBones[0].m_Bone.position;
				...
				if (parentExists) {
					...
					if (diffAngle > 0) {
						...
						if (i != m_LookAtBones.Length - 1) {
							...
							Vector3 firstBoneToTargetDiff = m_LookatTarget.transform.position - m_LookAtBones[0].m_Bone.position;
						...
						if (m_LookAtBones[i].m_LinkedBones.Length > 0) { linkedBonesRotations.Add(i, PerfectLookAtSlerp(Quaternion.identity, lookAtRot, m_Weight)); }
			...
			// Calculating the blend weight to blend between the current frame and last frame to achieve smooth rotations in dynamic
			// animations with large range of movements.

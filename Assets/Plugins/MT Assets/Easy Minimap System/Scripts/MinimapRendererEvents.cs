...
namespace MTAssets.EasyMinimapSystem
{
    ...
    */
    ...
    public class MinimapRendererEvents : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private bool isVirtualMouseOverTheMinimapRendererArea = false;
        private Vector3 startingWorldPositionOfOnPointerDownForCurrentOnDrag;

        //Cache variables
        private Vector2 currentVirtualMousePosition = new Vector2(0.5f, 0.5f);
        ...
        public virtual void OnPointerEnter(PointerEventData ped)
        {
            ...
            isVirtualMouseOverTheMinimapRendererArea = false;
        }
        ...
        public void Update()
        {
            ...
            if (isMouseOverTheMinimapRendererArea == false && isVirtualMouseOverTheMinimapRendererArea == false)
            {
            ...
            //On Input Over
            Vector2 mouseCoordinatesInEventsArea = Vector2.zero;
            if (isMouseOverTheMinimapRendererArea == true)
                mouseCoordinatesInEventsArea = GetPositionOfMouseInEventsAreaAndConvertToCoordinatesOfEventsArea();
            if (isVirtualMouseOverTheMinimapRendererArea == true)
                mouseCoordinatesInEventsArea = currentVirtualMousePosition;
        ...
        public void SimulateOnInputOverEvent(Vector2 moveAddiction)
        {
            //Add the move addiction to current position of virtual mouse
            currentVirtualMousePosition += moveAddiction;

            //Fix the values
            if (currentVirtualMousePosition.x < 0.0f)
                currentVirtualMousePosition.x = 0.0f;
            if (currentVirtualMousePosition.x > 1.0f)
                currentVirtualMousePosition.x = 1.0f;
            if (currentVirtualMousePosition.y < 0.0f)
                currentVirtualMousePosition.y = 0.0f;
            if (currentVirtualMousePosition.y > 1.0f)
                currentVirtualMousePosition.y = 1.0f;

            //Inform that virtual pointer is in minimap renderer area
            isMouseOverTheMinimapRendererArea = false;
            isVirtualMouseOverTheMinimapRendererArea = true;
        }

        public void SimulateOnInputDragEvent(Vector2 dragAddiction)
        {
            //Get the starting cursor position in world and the new cursor position in world
            Vector3 startingWorldPositionOfMouse = TranslateCoordinatesOfEventsAreaToWorldPosition(currentVirtualMousePosition);
            Vector3 currentWorldPositionOfMouse = TranslateCoordinatesOfEventsAreaToWorldPosition((currentVirtualMousePosition + dragAddiction * -1.0f));

            //Call the event
            if (minimapRenderer.onInputDrag != null)
                minimapRenderer.onInputDrag.Invoke(startingWorldPositionOfMouse, currentWorldPositionOfMouse);
        }

        public void SimulateOnInputClickEvent()
        {
            //On virtual pointer down
            Vector3 worldPositionOfMouse = TranslateCoordinatesOfEventsAreaToWorldPosition(currentVirtualMousePosition);
            MinimapItem minimapItemOfClick = TryToFindMinimapItemForInteractInThisWorldPosition(worldPositionOfMouse);

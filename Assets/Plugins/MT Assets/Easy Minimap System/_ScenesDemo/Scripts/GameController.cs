...
namespace MTAssets.EasyMinimapSystem
{
    public class GameController : MonoBehaviour
    {
        ...
        public MinimapRenderer playerMinimapRenderer;
        ...
        void Update()
        {
            ...
            //Keyboard inputs
            if (fullScreenMapObj.activeSelf == true && playerMinimapRenderer != null)
            {
                //Move cursor
                if (Input.GetKey(KeyCode.LeftShift) == false)
                {
                    if (Input.GetKey(KeyCode.W) == true)
                        playerMinimapRenderer.MoveVirtualCursor(new Vector2(0, (0.2f * Time.deltaTime)));
                    if (Input.GetKey(KeyCode.A) == true)
                        playerMinimapRenderer.MoveVirtualCursor(new Vector2((-0.2f * Time.deltaTime), 0));
                ...
                {
                    if (Input.GetKey(KeyCode.W) == true)
                        playerMinimapRenderer.ClickAndDragVirtualCursor(new Vector2(0, (5.0f * Time.deltaTime)));
                    if (Input.GetKey(KeyCode.A) == true)
                        playerMinimapRenderer.ClickAndDragVirtualCursor(new Vector2((-5.0f * Time.deltaTime), 0));
                    if (Input.GetKey(KeyCode.S) == true)
                        playerMinimapRenderer.ClickAndDragVirtualCursor(new Vector2(0, (-5.0f * Time.deltaTime)));
                    if (Input.GetKey(KeyCode.D) == true)
                        playerMinimapRenderer.ClickAndDragVirtualCursor(new Vector2((5.0f * Time.deltaTime), 0));
                ...
                if (Input.GetKeyDown(KeyCode.Return) == true)
                    playerMinimapRenderer.ClickWithVirtualCursor();
            }
        }
        ...
        IEnumerator OnClickInMinimapRendererArea_DoubleClickRoutine()
        {
            ...
            while (enabled)
            {
                if (milisecondsPassed >= 50) //<-- if is passed 50ms, reset the counter of clicks and break the loop
                {
